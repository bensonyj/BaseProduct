...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    for (int i = 0; i < self.numberOfStar; i ++)
    {
        ...
        NSInteger width = frame.size.height;
        imageView.frame = CGRectMake(i*(margin+width), 0, width, width);
        ...
        if(!imageView.image){
            ...
            if([imageName isEqualToString:kBACKGROUND_STAR]){
                [imageView setBackgroundColor:[UIColor colorWithRed:229/255.0 green:229/255.0 blue:0/255.0 alpha:1.0]];
            }
            else{
                [imageView setBackgroundColor:[UIColor colorWithRed:255/255.0 green:108/255.0 blue:38/255.0 alpha:1.0]];
            }
        }
    }
    return view;
...
//    self.starForegroundView.frame = CGRectMake(0, 0, p.x, self.frame.size.height);
    ...
    _score = ceilf(p.x/ (self.frame.size.height + margin));
    self.starForegroundView.frame = CGRectMake(0, 0, _score*(self.frame.size.height + margin), self.frame.size.height);
    
//    if(self.delegate && [self.delegate respondsToSelector:@selector(starRatingView: score:)] && !isManualSet)
    if(self.delegate && [self.delegate respondsToSelector:@selector(starRatingView:score:)] && !isManualSet)
    {
